...
function onURLChange() {
  ...
  if (newurl.includes("/shorts/")) {
    ...
    window.location.replace(
      window.location.origin + "/watch?v=" + videoLink[videoLink.length - 1]
    );
  }
}
...
function recordNodeUpdates(maxUpdates, callback) {
  ...
  createObserver(
    (m) => {
      m.forEach((record) => {
        numberOfUpdates += record.addedNodes.length;
      });
      if (numberOfUpdates > maxUpdates - 1) {
        updateBody();
        numberOfUpdates = 0;
        callback();
      }
